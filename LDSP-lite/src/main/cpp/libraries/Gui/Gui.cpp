...
#include "Gui.h"
#include "WebServer.h"
#include "files_utils.h" // to access assets and file system seamlessly
#include "LDSP_log.h"
...
public:
    ...
    std::shared_ptr<seasocks::Response> handle(const seasocks::Request& request) override {
        ...
        const std::string basePath = ""; // an empty base patch means "from the assets folder"

        LDSP_log("___________%s\n", uri.c_str());
        ...
        if (uri.find("/projects/") == 0) {
            ...
            if (endsWith(uri, "/main.html")) {
                filePath = basePath + "main.html";
            } else if (endsWith(uri, ".js")) {
                filePath = basePath + "sketch.js";
            }
        ...
        if (uri == "/" || uri == "/gui/index.html" || uri == "/gui/") {
            ...
            //LDSP_log("___________%sresources/gui/index.html\n", basePath.c_str());
...
private:
  std::shared_ptr<seasocks::Response> serveFile(const std::string& path, const std::string& mimeType) {
    auto content = readFile(path); //VIC unlike in LDSP, use readFile to get the file content
    //LDSP uses regular ifstream, which cannot be used to access assets on Android
    ...
    if (!content.empty()) {
      ...
      for (auto ch : content) {
        builder << ch; //VIC again, this is slightly different than in LDSP
      }
